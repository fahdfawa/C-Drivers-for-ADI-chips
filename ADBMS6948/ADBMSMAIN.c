...
//	   ADC_Cell_Voltage_Measurement(TOTAL_IC);
		ADBMS_Status_Reg_voltage_measurment(TOTAL_IC);
		 ADC_Cont_Current_Measurement(TOTAL_IC);
		MXC_Delay(3000000);
	 }
}
...
void printMenu(void)
{
  ...
  printf("2           : Measure cell voltages - Singleshot\n");
  printf("3           : Measure Current - Singleshot\n");
  printf("4           : Measure current - Continuous\n");
  printf("5           : Measure s-channel voltages\n");
  printf("6           : Measure average cell voltages\n");
  printf("7           : Measure gpio pin voltages\n");
  printf("8           : Measure status register voltages\n");
  printf("9           : Measure continuous synchronized voltage and current\n");
  printf("10          : Measure Coulumb counter and time Based Regs \n");
  printf("\n");
...
void run_command(int command)
{
 switch(command)
 {
  ...
  case 3:
 	  ADC_Singlehot_Current_Measurement(TOTAL_IC);
    break;
  ...
  case 9:
    ADC_Cont_Sync_V_and_I_Measurement(TOTAL_IC);
    break;

  case 10:
    ADC_Coulumb_Counter_Measurement(TOTAL_IC);
    break;

  case 11:
    ADC_Coulumb_Counter_Measurement_debugg(TOTAL_IC);
    break;
...
*/
void ADC_Cont_Current_Measurement(uint8_t TOTAL_IC)
{
	int   Reg_Size=RECEIVE_RD_PACKET_SIZE;
	uint8_t *Data_Read;
	Data_Read = (uint8_t *)calloc(Reg_Size, sizeof(uint8_t));
...
void ADBMS_Status_Reg_voltage_measurment(uint8_t TOTAL_IC)
{
  ...
  ADBMS_Print_Status_Data(TOTAL_IC, RDSTATF, Data_Read, GRPF);
...
void ADBMS_Print_Data(uint8_t tIC, uint8_t cmd_arg[], uint8_t *buff, RD_DATA_SIZE_ALL_TYPE type)
{
  ...
  for(uint8_t IC=0;IC<tIC;IC++)
  {
    ...
    z = twos_complement_to_int((buff[i+5]<< 8)|buff[i+4],16);
         ...
         double Cell1_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(x);
         double Cell2_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(y);
         double Cell3_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(z);
         double Cell4_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(x);
         double Cell5_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(y);
         double Cell6_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(z);
         double Cell7_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(x);
         double Cell8_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(y);
         double Cell9_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(z);
         double Cell10_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(x);
         double Cell11_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(y);
         double Cell12_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(z);
         double Cell13_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(x);
         double Cell14_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(y);
         double Cell15_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(z);
         double Cell16_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(x);

         double Pack_Voltage = (Cell1_volt + Cell2_volt + Cell3_volt + Cell4_volt + Cell5_volt + Cell6_volt + Cell7_volt + Cell8_volt + Cell9_volt + Cell10_volt + Cell11_volt + Cell12_volt + Cell13_volt + Cell14_volt + Cell15_volt + Cell16_volt);
    if			( type == GRPA){printf("\nIC:0%x->\t CELL1  =%1.3lf\t CELL2 =%1.3lf\t CELL3 =%1.3lf\n",(IC+1), Cell1_volt, Cell2_volt, Cell3_volt);}
    else if		( type == GRPB){printf("\nIC:0%x->\t CELL4  =%1.3lf\t CELL5 =%1.3lf\t CELL6 =%1.3lf\n",(IC+1), Cell4_volt, Cell5_volt, Cell6_volt);}
    else if 	( type == GRPC){printf("\nIC:0%x->\t CELL7  =%1.3lf\t CELL8 =%1.3lf\t CELL9 =%1.3lf\n",(IC+1), Cell7_volt, Cell8_volt, Cell9_volt);}
    else if 	( type == GRPD){printf("\nIC:0%x->\t CELL10 =%1.3lf\t CELL11=%1.3lf\t CELL12=%1.3lf\n",(IC+1), Cell10_volt, Cell11_volt, Cell12_volt);}
    else if 	( type == GRPE){printf("\nIC:0%x->\t CELL13 =%1.3lf\t CELL14=%1.3lf\t CELL15=%1.3lf\n",(IC+1), Cell13_volt, Cell14_volt, Cell15_volt);}
    else if 	( type == GRPF){printf("\nIC:0%x->\t CELL16 =%1.3lf\n",(IC+1), Cell16_volt);
                                printf("Pack_Voltage:  %f\n",(double) Pack_Voltage);}
    else	    {printf("error");}
    i +=6;
  }
}
...
void ADBMS_Print_Current_Data(uint8_t tIC, uint8_t cmd_arg[], uint8_t *buff, RD_DATA_SIZE_ALL_TYPE type)
{
  ...
  for(uint8_t IC=0;IC<tIC;IC++)
  {
    ...
    I1	=	ADBMS_CONVERT_CURRENT_HEX_TO_VOLT(a) / ((double)217 * (double)0.000001);
    I2	=	ADBMS_CONVERT_CURRENT_HEX_TO_VOLT(b) / ((double)217 * (double)0.000001);
...
void ADBMS_Print_Status_Data(uint8_t tIC, uint8_t cmd_arg[], uint8_t *buff, RD_DATA_SIZE_ALL_TYPE type)
{
  ...
  for(uint8_t IC=0;IC<tIC;IC++)
  {
    ...
    else if		( type == GRPF){printf("\nIC:0%x->\t  VM = %1.3lf\t Total Cell Voltage = %1.3lf\t VREF1= %1.3lf\n", (IC+1), ADBMS_CONVERT_VM_HEX_TO_VOLT(x),      ADBMS_CONVERT_VP_HEX_TO_VOLT(y),  ADBMS_CONVERT_VREF1_HEX_TO_VOLT(z));}
    else	    {printf("error");}
    i +=6;
  }
}
...
void ADBMS_Print_All_Reg_Data(uint8_t tIC, uint8_t cmd_arg[], uint8_t *buff, RD_DATA_SIZE_ALL_TYPE type)
{
  ...
  for(uint8_t IC=0;IC<tIC;IC++)
  {
    ...
    {
     double Cell1_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[0]);
     double Cell2_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[1]);
     double Cell3_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[2]);
     double Cell4_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[3]);
     double Cell5_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[4]);
     double Cell6_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[5]);
     double Cell7_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[6]);
     double Cell8_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[7]);
     double Cell9_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[8]);
     double Cell10_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[9]);
     double Cell11_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[10]);
     double Cell12_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[11]);
     double Cell13_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[12]);
     double Cell14_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[13]);
     double Cell15_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[14]);
     double Cell16_volt = ADBMS_CONVERT_CELLVOLTAGE_HEX_TO_VOLT(a[15]);
