...
#define ENABLE_PIN MXC_GPIO_PIN_3

#define CS2_PORT MXC_GPIO0
#define CS2_PIN MXC_GPIO_PIN_13
/***** Globals *****/
...
int main(void)
{
   ...
   //GENERAL SETTINGS
    ...
    Write_Register_TMC4671(TMC4671_MOTOR_TYPE_N_POLE_PAIRS, 0x00030003);         //Motor type and No : of pole pairs
...
//CLOSED LOOP SETTINGS
    ...
    // ADC configuration
	   ...
	   Write_Register_TMC4671(TMC4671_ADC_I_SELECT, 0x18000100);                   //Assigning correct ADC channel to PWM channel
	   ...
	   Write_Register_TMC4671(TMC4671_ADC_I0_SCALE_OFFSET, 0x01008191);            // Calibration of ADC channel 0
	   Write_Register_TMC4671(TMC4671_ADC_I1_SCALE_OFFSET, 0x01008181);            // Calibration of ADC channel 1
    ...
    // Feedback Selection and Configuration
	   ...
	   Write_Register_TMC4671(TMC4671_ABN_DECODER_MODE , 0x0000000);              //Configure Decoder input signals and count direction
	   Write_Register_TMC4671(TMC4671_ABN_DECODER_PPR, 0x00000FA0);                //Decoder pulses per mechanical revolution
	   Write_Register_TMC4671(TMC4671_ABN_DECODER_COUNT, 0x00000000);              //Raw Decoder count
	   Write_Register_TMC4671(TMC4671_ABN_DECODER_COUNT_N, 0X00000000);            //Count value at N pulse event
	   Write_Register_TMC4671(TMC4671_ABN_DECODER_PHI_E_PHI_M_OFFSET, 0xFC670000); //Offset setting for angle from dec
	   Write_Register_TMC4671(TMC4671_ABN_DECODER_PHI_E_PHI_M, 0xF985FF0A);        //Displays actual angle values for ABN encoder
	...
	//Flux & Torque PI parameters
	   ...
	   Write_Register_TMC4671(TMC4671_PID_FLUX_P_FLUX_I, 0x03E80320);              //PI parameters for Flux controller
	   Write_Register_TMC4671(TMC4671_PID_TORQUE_P_TORQUE_I, 0x03E80320);          //PI parameters for Torque controller
	   Write_Register_TMC4671(TMC4671_PID_TORQUE_FLUX_LIMITS, 0x00001388);           //Target current limit for Flux & Torque controller
	   Write_Register_TMC4671(TMC4671_PID_VELOCITY_LIMIT, 0x00000032);
	   Write_Register_TMC4671(TMC4671_PID_ACCELERATION_LIMIT, 0x000000C8);

	// Velocity & Position PI parameters
	   Write_Register_TMC4671(TMC4671_PID_VELOCITY_P_VELOCITY_I, 0x1F404E20);      //PI parameters for Velocity loop
	   Write_Register_TMC4671(TMC4671_PID_POSITION_P_POSITION_I, 0x00640000);      //PI parameters for Position loop
	  ...
	  do{
    	...
    	Write_Register_TMC4671(TMC4671_PID_POSITION_TARGET, 0x0040000);                                  //Position Target
